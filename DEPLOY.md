...
## 証明書の自動読み取り（Claude / 任意）

接種証明書の写真を選んだときに、接種日・ワクチン名・動物病院・次回予定日を
自動でフォームに入れる機能です。**未設定でも写真の添付と手入力は使えます。**

1. https://console.anthropic.com/ で API キーを発行する
2. Vercel の **Settings → Environment Variables** に追加する

| 変数 | 値 |
|---|---|
| `ANTHROPIC_API_KEY` | `sk-ant-...`（Secret にする） |
| `ANTHROPIC_MODEL` | 省略可。既定は `claude-opus-5` |

3. 手元でも試すなら `.env.local` にも同じ行を足す

### 送られるもの・送られないもの

- **送られる**: 証明書の画像そのもの（長辺2000pxのJPEGに縮小したもの）。
  **飼い主の氏名・住所・電話が写っていれば、それも画像として送られます。**
...
気になる場合は `ANTHROPIC_API_KEY` を設定しないでください。写真の添付だけが動きます。
