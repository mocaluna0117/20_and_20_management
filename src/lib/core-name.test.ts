...
describe("findCoreName — 不変条件", () => {
  ...
  it("材料まで含めて抽出する（家族語だけにしない）", () => {
    assert.equal(
      core("【朝9:00〜13:00限定販売♡30g増量♡】『血液検査安定サポート！』身体整う♡カンガルー肉ビッツ！"),
      "カンガルー肉ビッツ",
    );
    assert.equal(
      core("4月！『濃厚♪ジューシー♪ラム肉の五目炊き込みごはん♡』"),
      "ラム肉の五目炊き込みごはん",
    );
  });

  it("単語の途中から切り出さない", () => {
    // 「も」は助詞でもあるが「もちもち」の先頭でもある
    assert.equal(
      core("『もちもちポーク＋さつまいものクリーミーシチュー♡』"),
      "もちもちポーク＋さつまいものクリーミーシチュー",
    );
    // 上限で切れる場合も語の切れ目まで戻す
    const long = core(
      "ハーブ育ち若鶏＋コラーゲンパンプキンの炊き込み内臓ケアご飯♡手作りご飯4セットご注文のお客様に非売品ご飯プレゼント♡",
    );
    assert.ok(long && !/^[ーぁ-ゟ]/.test(long), `語の途中から始まっている: ${long}`);
  });
